...
GradjaninBiH::GradjaninBiH(std::string ime_i_prezime, long long int jmbg)
{
    ...
    for(int i = 0; i < 13; i++) {
        if(i == 0) c13 = jmbg_copy%10;
        else if(i == 1) c12 = jmbg_copy%10;
        else if(i == 2) c11 = jmbg_copy%10;
        else if(i == 3) c10 = jmbg_copy%10;
        else if(i == 4) c9 = jmbg_copy%10;
        else if(i == 5) c8 = jmbg_copy%10;
        else if(i == 6) c7 = jmbg_copy%10;
        else if(i == 7) c6 = jmbg_copy%10;
        else if(i == 8) c5 = jmbg_copy%10;
        else if(i == 9) c4 = jmbg_copy%10;
        else if(i == 10) c3 = jmbg_copy%10;
        else if(i == 11) c2 = jmbg_copy%10;
        else if(i == 12) c1 = jmbg_copy%10;
        jmbg_copy /= 10;
    }
    int dan = c1*10+c2;
    int mjesec = c3*10+c4;
    int godina = c5*100+c6*10+c7;
    if(godina >= 917) godina += 1000;
    else godina += 2000;
    int c = 11 - (7*(c1+c7) + 6*(c2+c8) + 5*(c3+c9) + 4*(c4+c10) + 3*(c5+c11) + 2*(c6+c12)) % 11;
    if(c == 11) c = 0;
    if(c13 != c) throw std::logic_error("JMBG nije validan");
    ...
    while(zadnji != nullptr) {
        if((*zadnji).JMBG == jmbg_copy) throw std::logic_error("Vec postoji gradjanin sa istim JMBG");
...
GradjaninBiH::GradjaninBiH(std::string ime_i_prezime, int dan_rodjenja, int mjesec_rodjenja, int godina_rodjenja, int sifra_regije, Pol pol)
{
    ...
    if(godina_rodjenja % 4 == 0 && godina_rodjenja % 100 != 0 || godina_rodjenja % 400 == 0)
        broj_dana[1]++;
    if(godina_rodjenja < 1 || mjesec_rodjenja < 1 || mjesec_rodjenja > 12 || dan_rodjenja < 1
            || dan_rodjenja > broj_dana[mjesec_rodjenja - 1])
        throw std::logic_error("Neispravni podaci");
    ime = ime_i_prezime;
    JMBG = dan_rodjenja * pow(10, 11) + mjesec_rodjenja*pow(10,9) + (godina_rodjenja%1000)*pow(10,6) + sifra_regije*10000;
    int kod = 0;
    if(pol == GradjaninBiH::Pol::Zensko) kod = 500;
    auto zadnji = posljednji;
    auto jmbg_copy = JMBG;
    int c1, c2, c3, c4, c5, c6, c7, c8, c9, c10, c11, c12, c13;
    if(zadnji == nullptr){
...
{
    if((JMBG % 10000)/10 < 500) return GradjaninBiH::Pol::Musko;
...
int main ()
{
    ...
    for(int i = 0; i < n; i++) {
        ...
        try {
            ...
            if(g.DajPol() == GradjaninBiH::Pol::Zensko) std::cout <<"zensko.";
            else std::cout << "musko.";
            std::cout << std::endl;
