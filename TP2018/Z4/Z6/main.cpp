...
public:
    Student(int brojindeksa, std::string godina, std::string ime, std::string adress, std::string telefon);
    int DajIndeks() const
    {
        return  broj_indeksa;
    }
    ...
    void Ispisi()
    {
        std::cout << "Broj indeksa: " << broj_indeksa << std::endl << "Godina studija: " << godina_studija << std::endl << "Ime i prezime: " << ime_i_prezime
                  << std::endl << "Adresa: " << adresa << std::endl << "Telefon: " << broj_telefona << std::endl;
    }
};
...
public:
    ...
    {
        for(auto x : ad.studenti) studenti[x.first] = std::make_shared<Student>(*x.second);
        for(auto x : ad.laptopi) laptopi[x.first] = std::make_shared<Laptop>(*x.second);
    }
    ...
    void RegistrirajNovogStudenta(int brojindeksa, std::string godina, std::string ime, std::string adress, std::string telefon)
    {
        if(studenti.find(brojindeksa) != studenti.end()) throw std::domain_error("Student s tim indeksom vec postoji");
        studenti[brojindeksa] = std::make_shared<Student>(brojindeksa, godina, ime, adress, telefon);
    }
    void RegistrirajNoviLaptop(int broj, std::string ime, std::string karakt)
    {
        if(laptopi.find(broj) != laptopi.end()) throw std::domain_error("Laptop s tim evidencijskim brojem vec postoji");
        (laptopi[broj]) = std::make_shared<Laptop>(broj, ime, karakt);
    }
    ...
    Student NadjiStudenta(int brojindeksa) const
    {
        ...
        if(studenti_copy.find(brojindeksa) == studenti_copy.end()) throw std::domain_error("Student nije nadjen");
    ...
    Laptop NadjiLaptop(int broj) const
    {
        ...
        if(laptopi_copy.find(broj) == laptopi_copy.end()) throw std::domain_error("Laptop nije nadjen");
    ...
    void ZaduziLaptop(int brojindeksa, int evbroj)
    {
        ...
        if(laptopi[evbroj]->Laptop::DaLiJeZaduzen()) throw std::domain_error("Laptop vec zaduzen");
        for(auto it = laptopi.begin(); it != laptopi.end(); it++)
            if((it->second)->Laptop::DaLiJeZaduzen() && (it->second)->Laptop::DajKodKogaJe().Student::DajImePrezime() == studenti[brojindeksa]->Student::DajImePrezime()) throw std::domain_error("Student je vec zaduzio laptop");
        laptopi[evbroj]->Laptop::Zaduzi(*studenti[brojindeksa]);
    }
    int NadjiSlobodanLaptop() const
    {
        for(auto it = laptopi.begin(); it != laptopi.end(); it++)
            if(!(it->second->Laptop::DaLiJeZaduzen())) return it->second->Laptop::DajEvidencijskiBroj();
        throw std::domain_error("Nema slobodnih laptopa");
    }
    void RazduziLaptop(int evbroj)
    {
        if(laptopi.find(evbroj) == laptopi.end()) throw std::domain_error("Laptop nije nadjen");
        if(!laptopi[evbroj]->Laptop::DaLiJeZaduzen()) throw std::domain_error("Laptop nije zaduzen");
    ...
    void PrikaziZaduzenja() const
    {
        bool zaduzen = false;
        for(auto it = laptopi.begin(); it != laptopi.end(); it++)
            if(it->second->Laptop::DaLiJeZaduzen()) zaduzen = true;
        if(!zaduzen) std::cout << "Nema zaduzenja" << std::endl;
        else
            for(auto it = laptopi.begin(); it != laptopi.end(); it++)
                if(it->second->Laptop::DaLiJeZaduzen())
                    std::cout << "Student " << it->second->Laptop::DajKodKogaJe().Student::DajImePrezime() << " (" << it->second->Laptop::DajKodKogaJe().Student::DajIndeks() << ") zaduzio/la laptop broj " << it->second->Laptop::DajEvidencijskiBroj() << std::endl;


    }
};

Student::Student(int brojindeksa, std::string godina, std::string ime, std::string adress, std::string telefon)
{
    int br = brojindeksa;
    for(int i = 0; i < 5; i++) {
        br /= 10;
    ...
    for(int i = 0; i < ime.length(); i++) {
        ...
        while(i!= ime.length()-1 && ime.at(i) != ' ') i++;
        while(i != 0 && i != ime.length() - 1 && ime.at(i) == ' ')
        if(ime.at(++i) == ' ')
                ime.erase(ime.begin()+(--i));
        if(i == ime.length() - 1 && ime.at(i) == ' ') ime.erase(ime.begin()+i);
    }
    ime_i_prezime = ime;
    for(int i = 0; i < adress.length(); i++) {
        ...
        while(i!= adress.length()-1 && adress.at(i) != ' ') i++;
        while(i != 0 && i != adress.length() - 1 && adress.at(i) == ' ')
        if(adress.at(++i) == ' ')
                adress.erase(adress.begin()+(--i));
        if(i == adress.length() - 1 && adress.at(i) == ' ') adress.erase(adress.begin()+i);
    }
    adresa = adress;
    for(int i = 0; i < telefon.length(); i++) {
        if(i == 0) {
            while(i != telefon.length() - 1 && telefon.at(i) >= '0' && telefon.at(i) <= '9') i++;
            if(i == telefon.length()-1) throw std::domain_error("Neispravni parametri");
